...
| `prepareBitmapSubtitleOCR` | true | With `prepareNativeSubtitles`, give bitmap tracks (PGS / DVB / DVD) an OCR-fed WebVTT rendition so they too survive PiP / AirPlay. Selecting one arms a second decode pass over the packet store plus a Vision request per cue and raises the forward-prefetch lead from 60 s to 270 s. Hosts that render bitmaps overlay-only can set false to skip that standing cost; text-track renditions are unaffected. |
...
| `diskCacheBudgetBytes` | nil | Explicit total budget for the loopback segment cache. A positive value enables byte-budgeted whole-source prefetch; the effective budget is capped at a quarter of temporary-volume free space and keeps a small AVPlayer safety window. `nil` or a non-positive value keeps automatic sizing. Takes precedence over `forwardBufferSegments`; ignored on `nativeRemoteHLS`. |
